...
export default function AboutHeaderManager() {
    ...
    const fetchData = async () => {
        try {
            const res = await fetch('/api/admin/about/header')
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault()
        try {
            const url = '/api/admin/about/header'
            ...
            if (res.ok) {
                ...
                setEditingItem(null)
                setFormData({ image: '', title: '', titleZh: '', height: 400, description: '', descriptionZh: '' })
                fetchData()
            } else {
                const errorData = await res.json()
                alert(`Failed to save header: ${errorData.error || 'Unknown error'}`)
            }
        } catch (error) {
            console.error('Failed to save header:', error)
            alert('Failed to save header. Please check the console for details.')
        }
    }
    ...
    const handleDelete = async (id: number) => {
        ...
        try {
            const res = await fetch(`/api/admin/about/header?id=${id}`, {
                method: 'DELETE'
            })
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <h2 className="text-xl font-bold">头部管理</h2>
                <button
                    onClick={() => {
                        setEditingItem(null)
                        setFormData({ image: '', title: '', titleZh: '', height: 400, description: '', descriptionZh: '' })
                    ...
                    <tbody className="divide-y divide-gray-200">
                        {headers.map((item) => (
                            <tr key={item.id} className="hover:bg-gray-50">
                                ...
                                <td className="px-6 py-4 text-sm text-gray-500">{item.height}px</td>

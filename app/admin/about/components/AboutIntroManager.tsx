...
export default function AboutIntroManager() {
    const [intros, setIntros] = useState<Intro[]>([])
    const [isLoading, setIsLoading] = useState(true)
    const [isModalOpen, setIsModalOpen] = useState(false)
    const [editingItem, setEditingItem] = useState<Intro | null>(null)
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        e.preventDefault()
        try {
            const url = '/api/admin/about/intro'
    ...
    const handleDelete = async (id: number) => {
        ...
        try {
            const res = await fetch(`/api/admin/about/intro?id=${id}`, {
                method: 'DELETE'
            })
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <h2 className="text-xl font-bold">简介管理</h2>
                <button
                    onClick={() => {
                        setEditingItem(null)
                        setFormData({ image: '', description: '', descriptionZh: '' })

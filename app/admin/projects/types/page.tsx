...
export default function ProjectTypesPage() {
    ...
    // Form states
    const [currentType, setCurrentType] = useState<Partial<ProjectType>>({ name: '' })
    ...
    const fetchData = async () => {
        setLoading(true)
        try {
            const res = await fetch('/api/admin/projects/types')
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        const url = currentType.id
            ? `/api/admin/projects/types/${currentType.id}`
            : '/api/admin/projects/types'
        ...
        try {
            ...
            if (res.ok) {
                ...
                setToast({ show: true, type: 'error', message: errorData.details || '保存失败，请重启服务器' })
    ...
    const confirmDelete = async () => {
        ...
        try {
            const res = await fetch(`/api/admin/projects/types/${typeToDelete}`, { method: 'DELETE' })
            if (res.ok) {
                    ...
                    <div className="bg-[#f0f2f5] w-full max-w-lg shadow-xl overflow-hidden rounded-sm animate-fade-in-up">
                        ...
                        <form onSubmit={handleSave} className="p-10 space-y-6">
                            <div className="flex items-center group">
                                <label className="w-24 text-right pr-4 text-gray-700 font-medium text-sm">项目类型:</label>
                    ...
                    <div className="bg-white w-full max-w-[400px] rounded shadow-xl overflow-hidden animate-fade-in-up">
                        ...
                        <div className="p-8 text-center bg-[#f9fafb]">
                            <p className="text-gray-600 text-sm leading-relaxed mb-8 px-4 font-medium">
                                删除该项目案例类型，项目案例管理列表将不再展示此内容，您确定要继续删除吗？
                            </p>

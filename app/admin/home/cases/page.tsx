...
export default function HomeCasesPage() {
    ...
    // Form states
    const [currentCase, setCurrentCase] = useState<Partial<HomeCase>>({})
    ...
    const fetchData = async () => {
        setLoading(true)
        try {
            const res = await fetch('/api/admin/home/cases')
            const data = await res.json()
            if (Array.isArray(data)) setCases(data)
        } catch (err) {
            console.error('Failed to fetch home cases:', err)
        } finally {
            setLoading(false)
        }
    }
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        const url = currentCase.id
            ? `/api/admin/home/cases/${currentCase.id}`
            : '/api/admin/home/cases'
    ...
    const confirmDelete = async () => {
        ...
        try {
            const res = await fetch(`/api/admin/home/cases/${caseToDelete}`, { method: 'DELETE' })
            if (res.ok) {
                        ...
                        {cases.map((c, index) => (
                            <tr key={c.id} className="hover:bg-gray-50 transition-colors">
                                ...
                                <td className="px-2 py-2 border-r border-gray-200">
                                    {c.image && (
                                        <img src={encodeURI(c.image)} alt="Case" className="h-8 w-8 object-contain mx-auto" />
                                    )}
                                </td>
                                <td className="px-4 py-3 border-r border-gray-200 text-gray-800 font-medium">{c.title}</td>
                                <td className="px-4 py-3 border-r border-gray-200 text-gray-600 truncate max-w-xs">{c.description}</td>
                                <td className="px-4 py-3 border-r border-gray-200 text-gray-600">{c.order}</td>
                                ...
                                <td className="px-4 py-3 border-r border-gray-200 text-gray-500 text-xs">
                                    {c.updatedAt
                                        ? new Date(c.updatedAt).toLocaleString('zh-CN', { hour12: false }).replace(/\//g, '-')
                                        : ''}
                                </td>
                        ...
                        <form onSubmit={handleSave} className="p-8 space-y-5">
                            ...
                            <div className="flex items-start">
                                ...
                                <div className="flex-1">
                                    ...
                                    >
                                        {currentCase.image ? (
                                            <img src={currentCase.image} alt="Case" className="w-full h-full object-contain p-2" />
                                        ) : (
                                            <>
                            ...
                            <div className="flex items-center">
                                ...
                                <div className="flex-1">
                                    <input
                                        ...
                                        className="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:outline-none focus:border-[#0ea5e9]"
                                        placeholder="请输入权重"
                                        value={currentCase.order || 0}
                                        onChange={e => setCurrentCase({ ...currentCase, order: parseInt(e.target.value) })}
                                    />
                                </div>
                            </div>

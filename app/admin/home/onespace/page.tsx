...
export default function OnespacePage() {
    const [items, setItems] = useState<OnespaceItem[]>([])
    ...
    // Form states
    const [currentItem, setCurrentItem] = useState<Partial<OnespaceItem>>({})
    ...
    const fetchData = async () => {
        setLoading(true)
        try {
            const res = await fetch('/api/admin/home/onespace')
            const data = await res.json()
            if (Array.isArray(data)) setItems(data)
        } catch (err) {
            console.error('Failed to fetch onespace items:', err)
        } finally {
            setLoading(false)
        }
    }
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        const url = currentItem.id
            ? `/api/admin/home/onespace/${currentItem.id}`
            : '/api/admin/home/onespace'
    ...
    const confirmDelete = async () => {
        ...
        try {
            const res = await fetch(`/api/admin/home/onespace/${itemToDelete}`, { method: 'DELETE' })
            if (res.ok) {

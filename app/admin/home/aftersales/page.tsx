...
export default function AfterSalesPage() {
    ...
    // Form states
    const [currentItem, setCurrentItem] = useState<Partial<AfterSalesItem>>({})
    ...
    const fetchData = async () => {
        setLoading(true)
        try {
            const res = await fetch('/api/admin/home/aftersales')
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        const url = currentItem.id
            ? `/api/admin/home/aftersales/${currentItem.id}`
            : '/api/admin/home/aftersales'
    ...
    const confirmDelete = async () => {
        ...
        try {
            const res = await fetch(`/api/admin/home/aftersales/${itemToDelete}`, { method: 'DELETE' })
            if (res.ok) {
                ...
                <table className="w-full text-center text-sm">
                    <thead className="bg-[#f8f9fa] border-b border-gray-300 text-gray-700 font-bold">
                        <tr>
                            ...
                            <th className="px-4 py-3 border-r border-gray-300 w-24">售后图</th>
                        ...
                        {items.map((item, index) => (
                            <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                                ...
                                <td className="px-2 py-2 border-r border-gray-200">
                                    {item.image && (
                                        <img src={item.image} alt="AfterSales" className="h-8 w-8 object-contain mx-auto" />
                                    )}
                                </td>
                    ...
                    <div className="bg-[#f0f2f5] w-full max-w-lg shadow-xl overflow-hidden rounded-sm animate-fade-in-up">
                        ...
                        <form onSubmit={handleSave} className="p-8 space-y-5">
                            ...
                            <div className="flex items-start">
                                <label className="w-24 text-right pr-4 text-gray-600 font-bold text-sm pt-2">售后图:</label>
                                <div className="flex-1">
                                    <div
                                    ...
                                    >
                                        {currentItem.image ? (
                                            <img src={currentItem.image} alt="AfterSales" className="w-full h-full object-contain p-2" />
                                        ) : (
                                            <>

...
function GlobalCollectContent() {
    const router = useRouter()
    const searchParams = useSearchParams()
    const [items, setItems] = useState<GlobalCollectItem[]>([])
    ...
    // Form states
    const [currentItem, setCurrentItem] = useState<Partial<GlobalCollectItem>>({})
    ...
    // Deep-link edit: /admin/home/global-collect?edit=ID
    useEffect(() => {
        const editId = searchParams.get('edit')
        if (!editId || items.length === 0) return
        const target = items.find(i => String(i.id) === editId)
        if (target) {
    ...
    const fetchData = async () => {
        setLoading(true)
        try {
            const res = await fetch('/api/admin/home/global-collect')
            const data = await res.json()
            if (Array.isArray(data)) setItems(data)
        } catch (err) {
            console.error('Failed to fetch global collect items:', err)
        } finally {
            setLoading(false)
        }
    }
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        const url = currentItem.id
            ? `/api/admin/home/global-collect/${currentItem.id}`
            : '/api/admin/home/global-collect'

        try {
            const res = await fetch(url, {
                method,
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ ...currentItem, order: currentItem.order ?? 0 })
            })
            if (res.ok) {
                setShowModal(false)
                fetchData()
    ...
    const confirmDelete = async () => {
        ...
        try {
            const res = await fetch(`/api/admin/home/global-collect/${itemToDelete}`, { method: 'DELETE' })
            if (res.ok) {
    ...
    const handleIconChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            const data = await res.json()
            if (data.url) {
                setCurrentItem(prev => ({ ...prev, icon: data.url }))
            }
        } catch (err) {
                ...
                <table className="w-full text-center text-sm">
                    <thead className="bg-[#f8f9fa] border-b border-gray-300 text-gray-700 font-bold">
                        <tr>
                            ...
                            <th className="px-4 py-3 border-r border-gray-300 w-28">背景图</th>
                            <th className="px-4 py-3 border-r border-gray-300 w-24">图标</th>
                            <th className="px-4 py-3 border-r border-gray-300 w-48">标题</th>
                            ...
                            <tr key={item.id} className="hover:bg-gray-50 transition-colors">
                                ...
                                <td className="px-2 py-2 border-r border-gray-200">
                                    {item.image ? (
                                        <img
                                            ...
                                            onError={(e) => { (e.currentTarget as HTMLImageElement).src = '/images/service_project.png' }}
                                        />
                                    ) : (
                                ...
                                <td className="px-4 py-3 text-sm">
                                    <div className="flex items-center justify-center gap-3">
                                        <Link
                                            href={`/admin/home/global-collect?edit=${item.id}`}
                                            onClick={(e) => {
                                                e.preventDefault()
                                                setCurrentItem(item)
                                                setShowModal(true)
                            ...
                            <button
                                ...
                                onClick={() => {
                                    ...
                                    if (searchParams.get('edit')) {
                                        router.replace('/admin/home/global-collect')
                                    }
                                }}
...
export default function GlobalCollectPage() {
    return (
        <Suspense fallback={<div className="p-6 text-gray-600">页面加载中...</div>}>
            <GlobalCollectContent />
        </Suspense>
    )
}

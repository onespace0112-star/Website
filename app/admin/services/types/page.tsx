...
export default function ServiceTypesPage() {
    ...
    // Form states
    const [currentType, setCurrentType] = useState<Partial<ServiceType>>({ name: '' })
    ...
    const handleSave = async (e: React.FormEvent) => {
        ...
        const url = currentType.id
            ? `/api/admin/services/types/${currentType.id}`
    ...
    const confirmDelete = async () => {
        ...
        try {
            const res = await fetch(`/api/admin/services/types/${typeToDelete}`, { method: 'DELETE' })
            if (res.ok) {
                ...
                <table className="w-full text-center text-sm">
                    <thead className="bg-[#f8f9fa] border-b border-gray-300 text-gray-700 font-bold">
                        <tr>
                            ...
                            <th className="px-4 py-3 border-r border-gray-300">服务类型</th>
                    ...
                    <div className="bg-[#f0f2f5] w-full max-w-lg shadow-xl overflow-hidden rounded-sm animate-fade-in-up">
                        ...
                        <form onSubmit={handleSave} className="p-10 space-y-6">
                            <div className="flex items-center group">
                                <label className="w-24 text-right pr-4 text-gray-700 font-medium text-sm">服务类型:</label>
